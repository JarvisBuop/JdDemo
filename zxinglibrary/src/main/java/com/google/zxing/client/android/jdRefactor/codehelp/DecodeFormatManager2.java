...
public final class DecodeFormatManager2 {
    ...
    public static Set<BarcodeFormat> getAllFormat(String decodeMode) {
        Set<BarcodeFormat> scanFormats = new HashSet<>();
        List<String> allMode = new ArrayList<>();
        allMode.add(Intents.Scan.ONE_D_MODE);
        allMode.add(Intents.Scan.PRODUCT_MODE);
        allMode.add(Intents.Scan.QR_CODE_MODE);
        allMode.add(Intents.Scan.DATA_MATRIX_MODE);
        allMode.add(Intents.Scan.AZTEC_MODE);
        allMode.add(Intents.Scan.PDF417_MODE);
        if (decodeMode == null) {
            try {
                for (String format : allMode) {
                    scanFormats.addAll(FORMATS_FOR_MODE.get(format));
                }
                return scanFormats;
            } catch (IllegalArgumentException iae) {
                // ignore it then
